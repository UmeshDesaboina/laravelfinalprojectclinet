            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between h-20 items-center">
                    ...
                    <div class="shrink-0 flex items-center">
                        <a href="<?php echo e(route('home')); ?>" class="text-2xl font-bold tracking-tight group">
                    ...
                    <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                        <a href="<?php echo e(route('home')); ?>" class="nav-link <?php echo e(request()->routeIs('home') ? 'active' : ''); ?> inline-flex items-center px-1 pt-1 text-sm font-medium leading-5">
                            Home
                        </a>
                        <a href="<?php echo e(route('shop')); ?>" class="nav-link <?php echo e(request()->routeIs('shop') ? 'active' : ''); ?> inline-flex items-center px-1 pt-1 text-sm font-medium leading-5">
                            Shop
                        </a>
                    ...
                    <div class="flex items-center gap-5">
                        ...
                        <div class="relative hidden md:block" x-data="{ query: '', results: [] }">
                            <input type="text" x-model="query" @input.debounce.300ms="fetch(`<?php echo e(route('api.search-suggestions')); ?>?q=${query}`).then(r => r.ok ? r.json() : []).then(d => results = d).catch(() => results = [])" placeholder="Search products..." class="w-64 rounded-full bg-dark-100 dark:bg-dark-700 border border-transparent focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 text-sm py-2.5 px-5 transition-all duration-300">
                            ...
                            <div x-show="query.length > 2 && results.length > 0" class="absolute top-full mt-3 w-full bg-white dark:bg-dark-800 rounded-2xl shadow-soft-lg border border-dark-100 dark:border-dark-700 p-3 z-50" x-cloak>
                                <template x-for="item in results" :key="item.id">
                                    <a :href="`<?php echo e(url('products')); ?>/${item.slug}`" class="flex items-center gap-3 p-3 hover:bg-dark-50 dark:hover:bg-dark-700 rounded-xl transition-colors duration-200">
                        ...
                        <?php if(auth()->guard()->check()): ?>
                            <div class="flex items-center gap-1">
                                <a href="<?php echo e(route('wishlist.index')); ?>" class="icon-btn relative" title="Wishlist">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path></svg>
                                </a>
                                <a href="<?php echo e(route('cart.index')); ?>" class="icon-btn relative" title="Cart">
                                ...
                                <div class="relative ml-2" x-data="{ open: false }">
                                    <button @click="open = !open" class="flex items-center gap-2 p-1 rounded-xl hover:bg-dark-100 dark:hover:bg-dark-700 transition-colors duration-300">
                                        <img src="https://ui-avatars.com/api/?name=<?php echo e(auth()->user()->name); ?>&background=22c55e&color=fff" class="w-9 h-9 rounded-xl border-2 border-transparent hover:border-primary-500 transition-colors duration-300" alt="">
                                    </button>
                                    <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" class="absolute right-0 mt-3 w-56 bg-white dark:bg-dark-800 rounded-2xl shadow-soft-xl border border-dark-100 dark:border-dark-700 py-3 z-50" x-cloak>
                                        <div class="px-4 py-2 border-b border-dark-100 dark:border-dark-700">
                                            <p class="text-sm font-semibold text-dark-800 dark:text-white"><?php echo e(auth()->user()->name); ?></p>
                                            <p class="text-xs text-dark-500"><?php echo e(auth()->user()->email); ?></p>
                                        </div>
                                        <?php if(auth()->user()->isAdmin()): ?>
                                            <a href="<?php echo e(route('admin.dashboard')); ?>" class="flex items-center gap-3 px-4 py-3 text-sm text-dark-600 dark:text-dark-300 hover:bg-dark-50 dark:hover:bg-dark-700 transition-colors">
                                        ...
                                        <?php endif; ?>
                                        <a href="<?php echo e(route('dashboard')); ?>" class="flex items-center gap-3 px-4 py-3 text-sm text-dark-600 dark:text-dark-300 hover:bg-dark-50 dark:hover:bg-dark-700 transition-colors">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path></svg>
                                            Dashboard
                                        </a>
                                        <a href="<?php echo e(route('profile.edit')); ?>" class="flex items-center gap-3 px-4 py-3 text-sm text-dark-600 dark:text-dark-300 hover:bg-dark-50 dark:hover:bg-dark-700 transition-colors">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                                            Profile
                                        </a>
                                        <a href="<?php echo e(route('orders.index')); ?>" class="flex items-center gap-3 px-4 py-3 text-sm text-dark-600 dark:text-dark-300 hover:bg-dark-50 dark:hover:bg-dark-700 transition-colors">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
                    <div class="col-span-1 md:col-span-1">
                        <a href="<?php echo e(route('home')); ?>" class="text-2xl font-bold tracking-tight mb-6 block">
...
<?php /**PATH C:\Users\umesh\fightwisdoml2\resources\views/layouts/app.blade.php ENDPATH**/ ?>
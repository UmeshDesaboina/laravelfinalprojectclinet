        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-gradient-to-r from-primary-500 to-primary-600 rounded-3xl p-8 mb-8 text-white relative overflow-hidden">
                ...
                <div class="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
                    ...
                    <div class="flex gap-4">
                        <a href="{{ route('shop') }}" class="px-6 py-3 bg-white text-primary-600 rounded-full font-bold hover:bg-primary-50 transition-all shadow-xl shadow-primary-500/20">
                            Continue Shopping
                        </a>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <div class="stat-card group">
                    ...
                    <p class="text-3xl font-black text-dark-800 dark:text-white">{{ auth()->user()->orders->count() }}</p>
                ...
                <div class="stat-card group">
                    ...
                    <p class="text-3xl font-black text-dark-800 dark:text-white">{{ auth()->user()->orders->whereIn('status', ['pending', 'processing', 'shipped'])->count() }}</p>
                ...
                <div class="stat-card group">
                    ...
                    <p class="text-3xl font-black text-dark-800 dark:text-white">{{ auth()->user()->orders->where('status', 'delivered')->count() }}</p>
                ...
                <div class="stat-card group">
                    ...
                    <p class="text-3xl font-black text-dark-800 dark:text-white">{{ auth()->user()->wishlists->count() ?? 0 }}</p>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2">
                    <div class="card-flat p-8">
                        <div class="flex justify-between items-center mb-6">
                            ...
                            <a href="{{ route('orders.index') }}" class="link-arrow text-sm">View All</a>
                        </div>
                        ...
                        @if($recentOrders->isEmpty())
                            <div class="text-center py-12">
                                ...
                                <a href="{{ route('shop') }}" class="btn-primary !py-3 !px-6 rounded-full inline-flex items-center gap-2">
                                ...
                                @foreach($recentOrders as $order)
                                    <div class="flex items-center justify-between p-5 bg-dark-50 dark:bg-dark-700/50 rounded-2xl hover:bg-dark-100 dark:hover:bg-dark-700 transition-colors group">
                                        <div class="flex items-center gap-4">
                                            ...
                                            <div>
                                                <p class="font-bold text-dark-800 dark:text-white">{{ $order->order_number }}</p>
                                                <p class="text-xs text-dark-400">{{ $order->created_at->format('M d, Y • h:i A') }}</p>
                                            </div>
                                        </div>
                                        <div class="text-right">
                                            <p class="font-black text-gradient bg-gradient-to-r from-primary-500 to-primary-600 bg-clip-text">₹{{ number_format($order->total, 2) }}</p>
                                        ...
                                        <a href="{{ route('orders.show', $order) }}" class="px-4 py-2 text-sm font-bold text-dark-400 hover:text-primary-500 transition-colors">
                                            View →
                                        </a>
                                    </div>
                                @endforeach
                            ...
                            <div class="mt-6 p-4 bg-gradient-to-r from-primary-50 to-accent-50 dark:from-primary-900/20 dark:to-accent-900/20 rounded-2xl">
                                <div class="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p class="text-2xl font-black text-gradient bg-gradient-to-r from-primary-500 to-primary-600 bg-clip-text">₹{{ number_format($totalSpent, 2) }}</p>
                                    </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="p-4 border border-dark-200 dark:border-dark-700 rounded-2xl">
                                <div class="flex items-center justify-between mb-4">
                                    <div>
                                        <p class="font-bold text-dark-800 dark:text-white">{{ $order->order_number }}</p>
                                        <p class="text-xs text-dark-500">{{ $order->created_at->format('M d, Y') }}</p>
                                    </div>
                                    <span class="status-badge 
                                ...
                                <div class="relative">
                                    <div class="flex items-center justify-between">
                                        ...
                                        <div class="flex-1 h-1 mx-2 rounded-full @if(in_array($order->status, ['processing', 'shipped', 'delivered'])) bg-green-500 @else bg-dark-200 dark:bg-dark-700 @endif"></div>
                                        <div class="flex flex-col items-center">
                                            <div class="w-8 h-8 rounded-full @if(in_array($order->status, ['processing', 'shipped', 'delivered'])) bg-green-500 @else bg-dark-200 dark:bg-dark-700 @endif flex items-center justify-center text-white text-xs font-bold">✓</div>
                                            <p class="text-xs text-dark-500 mt-1">Processing</p>
                                        </div>
                                        <div class="flex-1 h-1 mx-2 rounded-full @if(in_array($order->status, ['shipped', 'delivered'])) bg-green-500 @else bg-dark-200 dark:bg-dark-700 @endif"></div>
                                        <div class="flex flex-col items-center">
                                            <div class="w-8 h-8 rounded-full @if(in_array($order->status, ['shipped', 'delivered'])) bg-green-500 @else bg-dark-200 dark:bg-dark-700 @endif flex items-center justify-center text-white text-xs font-bold">✓</div>
                                            <p class="text-xs text-dark-500 mt-1">Shipped</p>
                                        </div>
                                        <div class="flex-1 h-1 mx-2 rounded-full @if($order->status === 'delivered') bg-green-500 @else bg-dark-200 dark:bg-dark-700 @endif"></div>
                                        <div class="flex flex-col items-center">
                                            <div class="w-8 h-8 rounded-full @if($order->status === 'delivered') bg-green-500 @else bg-dark-200 dark:bg-dark-700 @endif flex items-center justify-center text-white text-xs font-bold">✓</div>
                                ...
                                <div class="mt-4 pt-4 border-t border-dark-200 dark:border-dark-700">
                                    <p class="text-xs text-dark-500">Tracking: <span class="font-bold text-dark-800">{{ $order->tracking_id }}</span></p>
                                </div>
                                @endif
                            </div>
                            @endforeach
                ...
                <div class="lg:col-span-1">
                    <div class="card-flat p-8 mb-6">
                        ...
                        <div class="space-y-3">
                            <a href="{{ route('orders.index') }}" class="flex items-center gap-4 p-4 bg-dark-50 dark:bg-dark-700/50 rounded-xl hover:bg-primary-50 dark:hover:bg-primary-900/20 transition-colors group">
                            ...
                            <a href="{{ route('wishlist.index') }}" class="flex items-center gap-4 p-4 bg-dark-50 dark:bg-dark-700/50 rounded-xl hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors group">
                            ...
                            <a href="{{ route('profile.edit') }}" class="flex items-center gap-4 p-4 bg-dark-50 dark:bg-dark-700/50 rounded-xl hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-colors group">
                            ...
                            <a href="{{ route('addresses.index') }}" class="flex items-center gap-4 p-4 bg-dark-50 dark:bg-dark-700/50 rounded-xl hover:bg-green-50 dark:hover:bg-green-900/20 transition-colors group">
                    ...
                    <div class="card-flat p-8">
                        ...
                        <div class="space-y-4">
                            <div class="flex items-center gap-3">
                                <img src="https://ui-avatars.com/api/?name={{ auth()->user()->name }}&background=22c55e&color=fff&size=64" class="w-14 h-14 rounded-xl border-2 border-primary-500 shadow-lg shadow-primary-500/20" alt="">
                                <div>
                                    <p class="font-bold text-dark-800 dark:text-white">{{ auth()->user()->name }}</p>
                            ...
                            <div class="border-t border-dark-100 dark:border-dark-700 pt-4">
                                ...
                                <p class="font-bold text-dark-800 dark:text-white">{{ auth()->user()->created_at->format('M d, Y') }}</p>
                            </div>
                            @if(auth()->user()->addresses->count() > 0)
                            <div class="border-t border-dark-100 dark:border-dark-700 pt-4">
                                <p class="text-sm text-dark-500 mb-2">Default Address</p>
                                <p class="font-bold text-dark-800 dark:text-white text-sm">{{ auth()->user()->addresses->where('is_default', true)->first()->full_name ?? 'Not set' }}</p>
                                <p class="text-xs text-dark-400">{{ auth()->user()->addresses->where('is_default', true)->first()->city ?? '' }}, {{ auth()->user()->addresses->where('is_default', true)->first()->state ?? '' }}</p>
                            </div>
                            @endif
                            <!-- Security -->
                            <div class="border-t border-dark-100 dark:border-dark-700 pt-4">
                                <p class="text-sm text-dark-500 mb-3">Security</p>
                                <a href="{{ route('profile.edit') }}" class="flex items-center gap-2 text-sm text-primary-500 hover:text-primary-600 font-medium">

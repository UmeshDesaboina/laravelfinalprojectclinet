        ...
        <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-dark-100 dark:divide-dark-700">
                @foreach($orders as $order)
                    <tr class="table-row">
                        <td class="py-4 px-6">
                            <span class="font-bold text-dark-800 dark:text-white">{{ $order->order_number }}</span>
                            <span class="text-xs text-dark-400 block mt-1">{{ $order->created_at->format('M d, Y h:i A') }}</span>
                        </td>
                        <td class="py-4 px-6">
                            <div class="flex items-center gap-3">
                                <img src="https://ui-avatars.com/api/?name={{ $order->user->name }}&background=6366f1&color=fff&size=40" class="w-10 h-10 rounded-xl" alt="">
                                <div class="flex flex-col">
                                    <span class="font-medium text-dark-800 dark:text-white">{{ $order->user->name }}</span>
                                    <span class="text-xs text-dark-400">{{ $order->user->email }}</span>
                                </div>
                            </div>
                        </td>
                        <td class="py-4 px-6">
                            <span class="font-bold text-dark-800 dark:text-white">₹{{ number_format($order->total, 2) }}</span>
                        </td>
                        <td class="py-4 px-6">
                            <span class="badge {{ $order->payment_status == 'paid' ? 'badge-primary' : 'bg-red-100 text-red-700 dark:bg-red-900/30' }}">
                        ...
                        <td class="py-4 px-6 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.orders.show', $order) }}" class="p-2 rounded-xl bg-blue-50 dark:bg-blue-900/20 text-blue-600 hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-all duration-300">

                    ...
                    <tr class="table-row">
                        <td class="py-4 px-6">
                            <div class="flex items-center gap-4">
                                <img src="https://ui-avatars.com/api/?name={{ $user->name }}&background=6366f1&color=fff&size=48" class="w-12 h-12 rounded-xl" alt="">
                                <div class="flex flex-col">
                                    <span class="font-semibold text-dark-800 dark:text-white">{{ $user->name }}</span>
                                    <span class="text-xs text-dark-400">{{ $user->email }}</span>
                                </div>
                            </div>
                        </td>
                        <td class="py-4 px-6">
                            <span class="text-sm text-dark-600 dark:text-dark-400">{{ $user->email }}</span>
                        </td>
                        <td class="py-4 px-6">
                            <span class="text-sm text-dark-600 dark:text-dark-400">{{ $user->created_at->format('M d, Y') }}</span>
                        </td>
                        <td class="py-4 px-6">
                            @if($user->orders && $user->orders->count() > 0)
                                <div class="flex flex-col gap-1">
                                    @foreach($user->orders as $order)
                                        <span class="text-xs font-medium text-primary-600 dark:text-primary-400">{{ $order->order_number }}</span>
                                    @endforeach
                                </div>
                            @else
                        ...
                        <td class="py-4 px-6">
                            <button onclick="toggleBlock({{ $user->id }})" id="block-btn-{{ $user->id }}" 
                                class="badge {{ $user->is_blocked ? 'bg-red-100 text-red-700 dark:bg-red-900/30' : 'badge-primary' }}">
                                {{ $user->is_blocked ? 'Blocked' : 'Active' }}
                            </button>
                        </td>
                        <td class="py-4 px-6 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.users.show', $user) }}" class="p-2 rounded-xl bg-blue-50 dark:bg-blue-900/20 text-blue-600 hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-all duration-300">
